...
def populate_db():
	...
	for hit in r['hits']:
		...
		row = VideoData(title = hit['title'].decode('utf-8', 'ignore'), url = hit['url'].decode('utf-8', 'ignore'), 
			time = hit['created_at_i'], points = hit['points'], created_at = date[0].decode('utf-8', 'ignore'), 
			source = sourceAndImage['source'].decode('utf-8', 'ignore'), 
			image = sourceAndImage['imgSrc'].decode('utf-8', 'ignore'))
		session.add(row)
		session.commit()
	try:
		session.commit()
